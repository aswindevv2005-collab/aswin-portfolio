...
import { Send, Mic, Bot, User, MicOff, X, ExternalLink, Mail, MessageCircle, Phone } from "lucide-react";
...
export default function AIChatbot() {
  ...
  const handleSend = async (text: string) => {
    ...
    if (lower === "time" || lower === "what is the time") {
      ...
      botMsg.text = "Aswin Dev's father is Venu pillai K and his mother is Gaythri devi V. He also has a younger brother named Kashi Dev. Here is their family photo!";
  ...
  return (
    <div className="w-full max-w-4xl mx-auto flex flex-col h-[94dvh] md:h-[88vh] relative z-10 pt-4 md:pt-10 px-4">
      ...
      {/* Header */}
      <motion.div initial={{ y: -20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="flex items-center gap-3 mb-4 md:mb-6 mt-4 md:mt-0">
        ...
        <div>
          ...
          <p className="text-[10px] md:text-xs text-cyan-500/80 font-mono tracking-widest uppercase">AI-Powered Portfolio • Ask me anything</p>
        </div>
        ...
        <AnimatePresence>
          {messages.map((msg) => (
            <motion.div key={msg.id} initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className={`flex w-full items-start gap-3 ${msg.sender === "user" ? "flex-row-reverse" : ""}`}>
              <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${msg.sender === "bot" ? "bg-cyan-950/80 border border-cyan-500/30 text-cyan-400" : "bg-indigo-950/80 border border-indigo-500/30 text-indigo-400"}`}>
                {msg.sender === "bot" ? <Bot size={16} /> : <User size={16} />}
              </div>
              <div className={`max-w-[80%] p-4 rounded-2xl ${msg.sender === "user" ? "bg-cyan-600/20 border border-cyan-500/30 text-cyan-50 shadow-[0_0_15px_rgba(6,182,212,0.15)] rounded-tr-sm" : "bg-white/5 backdrop-blur-md border border-white/10 text-gray-200 rounded-tl-sm shadow-xl"}`}>
